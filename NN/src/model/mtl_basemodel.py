...
class MyDataset(Data.TensorDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.y != None:
            return (torch.cat([self.x[index]]+feature_list), self.y[index], torch.cat([self.x[index]]+feature_list), self.y[index])
        else:
            return (torch.cat([self.x[index]]+feature_list), torch.tensor([0]), torch.cat([self.x[index]]+feature_list), torch.tensor([0]))
...
class MyBaseModel(BaseModel):
    ...
    def fit(self, x=None, y=None, extra_data=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        for epoch in range(initial_epoch+1, epochs+1):
            ...
            tloss = []
            for step, (x1, y1, x2, y2) in enumerate(bar):
                x1 = x1.to(self.device).float()
                x2 = x2.to(self.device).float()
                y1 = y1.to(self.device).float()
                y2 = y2.to(self.device).float()
                y_pred_merge, y_pred_1, y_pred_2 = model(x1, x2)
                optim.zero_grad()
                loss = loss_func(torch.cat(y_pred_1, dim=1), y1, reduction='mean') + loss_func(
                    torch.cat(y_pred_2, dim=1), y2, reduction='mean') + loss_func(y_pred_merge, y1, reduction='mean')
    ...
    def predict(self, x, extra_data, batch_size=256):
        ...
        with torch.no_grad():
            for _, x_test in enumerate(tqdm(test_loader)):
                ...
                x2 = x_test[2].to(self.device).float()
                y_pred, _, _ = model(x1, x2, test=True)
